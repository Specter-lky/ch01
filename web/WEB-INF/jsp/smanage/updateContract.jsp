    ...
    <div class="hrms_body" style="position:relative; top:-15px;">
        ...
        <div class="emp_info col-sm-10">
            ...
            <%
                ...
                List<Orders> orders=(List) session.getAttribute("orders");
                DateFormat format = new SimpleDateFormat("yyyy-MM-dd");
                String date = format.format(contract.getCt_date());
            %>
            <form style="text-align: left;">
                <div class="msg">
                    合 同 号 ：<%=contract.getC_name()%>
                </div>
                <div class="msg">
                    负责员工：<%=contract.getS_name()%>
                </div>
                <div class="msg">
                    客户姓名：<%=contract.getC_name()%>
                </div>
                <div class="msg">
                    签订日期：<%=date%>
                </div>
                ...
                <table class="table" border="1" align="center">
                    ...
                    <tbody>
                        ...
                        <tr id="clo">
                        <td>
                            <input name="ono" value="<%=o.getO_no()%>" type="text" readonly="readonly">
                        </td>
                        <td style="width: 500px;">
                            <%=o.getG_name()%>
                        </td>
                        <td> <input name="onum" type="text" value="<%=o.getO_num()%>" onkeyup="this.value=this.value.replace(/\D/g, '')"/></td>
                        </tr>
                        <%
                            }
                        %>
...
<script type="text/javascript">
    $(".button").click(function ( ) {
        var $ono=$("input[name='ono']");
        var $onum = $("input[name='onum']");
        var $orders = new Array($ono.length);
        for (var i = 0; i < $ono.length; i++) {
            if ($onum.eq(i).val()=="") {
                alert("请填写订购数量");
                return false;
            }
            $orders[i] = {"ono": parseInt($ono.eq(i).val()),"onum": parseInt($onum.eq(i).val())}
        }
        var json = {"orders": $orders};
        var url = "http://localhost:8080/contract/updateContract";
        $.ajax({
            async: false,
            url: url,
            type: "POST",
